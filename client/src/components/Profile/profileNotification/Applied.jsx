...
const Applied = ({ notis }) => {
        ...
        const cancelHandler = async(e)=>{
            const {name} = e.target
            dispatch(notificationAction.setFunction({functionMessage:'canceling you challenge'}))
            try {
                const res = await axios.get(`http://localhost:3000/kheloNITH/notification/event/cancel/${name}`) 
                if(res.data.error){

...
const Received = ({ notis }) => {
    ...
    const rejectHandler = async(e)=>{
       ...
       const id= name
       try {
            const res = await axios.get(`http://localhost:3000/kheloNITH/notification/event/reject/${id}`)
    ...
    const acceptHandler = async(e)=>{
        ...
        const id= name
        try {
             const res = await axios.get(`http://localhost:3000/kheloNITH/notification/event/accept/${id}`)
